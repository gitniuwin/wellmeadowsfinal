    ...
    <form action="{{ route('treatments.update', $treatment->id) }}" method="POST">
      ...
      <div class="form-grid">
        <div class="form-group">
          ...
          <select name="patient_id" id="patient_id" required>
            @foreach($patients as $patient)
              <option value="{{ $patient->id }}" {{ $treatment->patient_id == $patient->id ? 'selected' : '' }}>{{ $patient->full_name }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="form-group">
          ...
          <select name="doctor_id" id="doctor_id" required>
            @foreach($doctors as $doctor)
              <option value="{{ $doctor->id }}" {{ $treatment->doctor_id == $doctor->id ? 'selected' : '' }}>{{ $doctor->full_name }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="form-group">
          ...
          <select name="appointment_id" id="appointment_id">
            ...
            @foreach($appointments as $appt)
              <option value="{{ $appt->id }}" {{ $treatment->appointment_id == $appt->id ? 'selected' : '' }}>{{ $appt->patient->full_name ?? '' }} - {{ \Carbon\Carbon::parse($appt->appointment_date)->format('M d, Y') }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="form-group">
          ...
          <input type="date" name="treatment_date" id="treatment_date" required value="{{ $treatment->treatment_date->format('Y-m-d') }}">
        </div>

        <div class="form-group full">
          <label for="diagnosis">Diagnosis</label>
          <input type="text" name="diagnosis" id="diagnosis" required value="{{ $treatment->diagnosis }}">
        </div>

        <div class="form-group full">
          <label for="procedure">Procedure</label>
          <input type="text" name="procedure" id="procedure" required value="{{ $treatment->procedure }}">
        </div>

        <div class="form-group full">
          <label for="notes">Notes</label>
          <textarea name="notes" id="notes" rows="3">{{ $treatment->notes }}</textarea>
        </div>
      </div>
