...
<div style="padding: 28px 36px;">
  <div style="background: white; border: 1px solid #C8D9EE; border-radius: 12px; padding: 28px; max-width: 600px;">
    <h2 style="font-family: 'Playfair Display', serif; font-size: 24px; margin-bottom: 6px; color: #1a2640;">Schedule New Appointment</h2>
    <p style="font-size: 13px; color: #6B7E9F; margin-bottom: 20px;">Fill in appointment details below.</p>
    ...
    <form method="POST" action="{{ route('appointments.store') }}">
      ...
      <div style="display: flex; flex-direction: column; gap: 14px;">
        <div>
          ...
          @error('patient_id') <span style="color: #D94F4F; font-size: 11px;">{{ $message }}</span> @enderror
        </div>
        ...
        <div>
          ...
          @error('doctor_id') <span style="color: #D94F4F; font-size: 11px;">{{ $message }}</span> @enderror
        </div>
        ...
        <div>
          ...
          <input type="datetime-local" name="appointment_date" required style="border: 1.5px solid #C8D9EE; border-radius: 7px; padding: 8px 12px; font-size: 13px; width: 100%;" />
        </div>

        <div>
          <label style="font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; color: #1a2640; font-weight: 500;">Notes</label>
          <textarea name="notes" style="border: 1.5px solid #C8D9EE; border-radius: 7px; padding: 8px 12px; font-size: 13px; width: 100%; resize: vertical;" rows="3"></textarea>
        </div>
        ...
        <div style="display: flex; gap: 10px; justify-content: flex-end; margin-top: 14px;">
          ...
          <button type="submit" style="padding: 9px 18px; border: none; border-radius: 8px; background: #1B2D5B; color: white; font-size: 13px; font-weight: 500; cursor: pointer;">Schedule</button>
        </div>
      </div>
    </form>
  </div>
